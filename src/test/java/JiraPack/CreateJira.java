...
//Login Scenario
        ...
        String response=given().relaxedHTTPSValidation().header("Content-Type"
                ,"application/json").body("{\r\n" +
                "    \"username\": \"hsindia2020\",\r\n" +
                "    \"password\": \"Harmeet1!\"\r\n" +
...
//Add comment
        String addCommentResponse = given().pathParam("key", "10005")
                .log().all().header("Content-Type","application/json").body("{\r\n" +
...
//Add Attachment
        ...
        given().header("X-Atlassian-Token","no-check")
                .filter(session).pathParam("key", "10005")
